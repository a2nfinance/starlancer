...
export const DeveloperManagers = () => {
    const { memberManagersForm } = useAppSelector(state => state.daoForm)
    const dispatch = useAppDispatch();
    const [form] = Form.useForm();
    const onFinish = (values: any) => {
        dispatch(setDaoFormProps({att: "memberManagersForm", value: values}))
        dispatch(setDaoFormProps({ att: "currentStep", value: 3 }))
    };
    return (
        <Form
            form={form}
            name='member-managers-form'
            initialValues={memberManagersForm}
            onFinish={onFinish}
            layout='vertical'
            autoComplete="off"
        >
            <Card title="Developer managers" headStyle={headStyle} extra={
                <Space>
