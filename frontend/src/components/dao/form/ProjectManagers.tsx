...
export const ProjectManagers = () => {
    const { projectManagersForm } = useAppSelector(state => state.daoForm)
    const dispatch = useAppDispatch();
    const [form] = Form.useForm();
    const onFinish = (values: any) => {
        dispatch(setDaoFormProps({ att: "projectManagersForm", value: values }))
        dispatch(setDaoFormProps({ att: "currentStep", value: 4 }))
    };
    return (
        <Form
            form={form}
            name='project-managers-form'
            initialValues={projectManagersForm}
            onFinish={onFinish}
            layout='vertical'
            autoComplete="off"
        >
            <Card title="Project managers" headStyle={headStyle} extra={
                <Space>
                    <Button type="primary" size='large' onClick={() => dispatch(setDaoFormProps({ att: "currentStep", value: 2 }))}>Back</Button>
