...
export const TreasuryManagers = () => {
    const { treasuryManagersForm } = useAppSelector(state => state.daoForm)
    const dispatch = useAppDispatch();
    const [form] = Form.useForm();
    const onFinish = (values: any) => {
        dispatch(setDaoFormProps({ att: "treasuryManagersForm", value: values }))
        dispatch(setDaoFormProps({ att: "currentStep", value: 2 }))
    };
    return (
        <Form
            form={form}
            name='treasury-managers-form'
            initialValues={treasuryManagersForm}
            onFinish={onFinish}
            layout='vertical'
            autoComplete="off"
        >
            <Card title="Treasury managers" headStyle={headStyle} extra={
                <Space>
